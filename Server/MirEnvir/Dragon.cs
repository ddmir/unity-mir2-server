    ...
    public class Dragon
    {
        ...
        public bool Load()
        {
            try
            {
                ...
                if (info == null)
                {
                    MessageQueue.Enqueue("无法加载Dragon（怪物名称错误）： " + Info.MonsterName);
                    return false;
                }
                ...
                if (map == null)
                {
                    MessageQueue.Enqueue("无法加载Dragon（错误的地图名称）： " + Info.MapFileName);
                    return false;
                }

                if (Info.Location.X > map.Width || Info.Location.Y > map.Height)
                {
                    MessageQueue.Enqueue("无法加载Dragon（错误的地图XY）：" + Info.MapFileName);
                    return false;
                }
            ...
            MessageQueue.Enqueue("无法加载Dragon");
            return false;
        }
